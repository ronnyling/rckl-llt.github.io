*** Settings ***
Resource          ${EXECDIR}${/}tests/restAPI/common.robot
Library           ${EXECDIR}${/}resources/restAPI/Config/AppSetup/AppSetupGet.py
Library           ${EXECDIR}${/}resources/restAPI/Config/AppSetup/AppSetupPut.py

Test Setup     run keywords
...    Given user retrieves token access as ${user_role}
...    user retrieves details of application setup

Test Teardown    run keywords
...   user revert to previous setting
...   expected return status code 200

*** Test Cases ***
1 - Able to update the App Setup - Customer Transfer records
    [Documentation]    Able to update the App Setup - Customer Transfer records
    [Tags]     hqadm   9.1
    ${AppSetupDetails}=    create dictionary
    ...     CUST_OPEN_ORDER=${True}
    ...     CUST_UNCONFIRMED_INVOICE=${True}
    ...     CUST_UNCONFIRMED_RETURN=${True}
    ...     CUST_UNCONFIRMED_EXCHANGE=${True}
    ...     CUST_UNCONFIRMED_CN_DN=${True}
    ...     CUST_ASSIGNED_TRADE_ASSET=${True}
    set test variable   &{AppSetupDetails}
    Given user retrieves token access as ${user_role}
    When user updates app setup details using fixed data
    Then expected return status code 200