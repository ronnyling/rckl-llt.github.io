...
*** Test Cases ***
1 - Able to search Sales Order using principal field
    [Documentation]    Able to search Sales Order with principal field when multi principal = On
    [Tags]     distadm  9.1    NRSZUANQ-30048
    Given user switches On multi principal
    And user navigates to menu Customer Transaction | Sales Order
    When user searches sales order with random data
    And principal listed successfully with searched data

2 - Validate type column is displayed when Sampling = On and Combine = No
    [Documentation]    Validate in sales order listing that type field is displayed when Sampling = Off and Combine = No
    [Tags]     distadm    9.3
...
3 - Validate type column is not displayed when Sampling = On and Combine = Yes
    [Documentation]    Validate in sales order listing that type field is not displayed when Sampling = Off and Combine = Yes
    [Tags]     distadm    9.3
    ${AppSetupDetails}=    create dictionary
...
4 - Validate type column is not displayed when Sampling = Off
    [Documentation]    Validate in sales order listing that type field is not displayed when Sampling = Off
    [Tags]     distadm    9.3
    [Setup]      run keywords
...
5   [Documentation]    Able to search sales order with type field when Sampling = Off and Combine = No
    [Tags]     distadm    9.3
    ${AppSetupDetails}=    create dictionary
    ...    SAMPLING_COMBINE_IN_SELLING_TXN=${False}
    set test variable   &{AppSetupDetails}
    ...
    And user open browser and logins using user role ${user_role}
    When user navigates to menu Customer Transaction | Sales Order
    And user searches sales order with Sampling data
    Then validate sampling order listed successfully
    When user searches sales order with Selling data
    Then validate sampling order listed successfully