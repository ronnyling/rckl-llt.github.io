...
class CompCreditNoteNonProductListPage(PageObject):
    """ Functions in Company Credit Note non product listing page """
    ...
    @keyword('user selects company credit note non product to ${action}')
    def select_company_cnnp(self, action):
        ...
        cnnp_list = ["CNNP_NO"]
        data_list = [cnnp_no]
        PAGINATION.validate_the_data_is_in_the_table_and_select_to("present", "Company Credit Note Non Product", action, cnnp_list,
                                                                   data_list)
