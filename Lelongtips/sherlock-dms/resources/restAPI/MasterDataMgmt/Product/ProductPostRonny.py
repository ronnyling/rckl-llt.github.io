    ...
    def payload_product_master_info(self, user):
        ...
        for x in range((len(prd_hier) - 1), -1, -1):
            ...
            if parent_id is None:
                rand = secrets.randbelow(len(tree_list))
                while not tree_list[rand]["children"]:
                    rand = secrets.randbelow(len(tree_list))
                node_id = tree_list[rand]["children"][0]["nodeId"]
                node_id_list.append(node_id)
                parent_id = tree_list[rand]["nodeId"]
            else:
                for i in tree_list:
        ...
        if update_details is not None and "SELLING_IND" not in update_details.keys():
