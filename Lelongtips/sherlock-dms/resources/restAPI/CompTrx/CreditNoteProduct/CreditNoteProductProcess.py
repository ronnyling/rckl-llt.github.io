...
class CreditNoteProductProcess(object):

    @keyword('user confirms created credit note product')
    def user_confirms_credit_note_product(self):
        scnp_id = BuiltIn().get_variable_value("${scnp_id}")
        url = "{0}supplier-cn-prd-header/{1}".format(END_POINT_URL, scnp_id)
        payload = CreditNoteProductPost.CreditNoteProductPost().payload("edit", "SAVE AND CONFIRM")
